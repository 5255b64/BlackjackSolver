...
pub fn spawn_dealer_cards(
    ...
    // // only for test
    // let cards = vec![
    //     ECard{ color: ECardColor::Hearts, value: ECardNumber::Ace },
    //     ECard{ color: ECardColor::Hearts, value: ECardNumber::King },
    //     ];
...
pub fn spawn_new_dealer_card(
    ...
    asset_server: &Res<AssetServer>,
    dealer_hand: &mut ResMut<DealerHand>,
    card: ECard,
) {
    let cards = &mut dealer_hand.cards;
    let idx = cards.len();
    let mut card_boundle = get_card_sprite_boundle(&card.color, &card.value, &asset_server);
    card_boundle.transform = Transform::from_xyz(X_START + X_STEP * idx as f32, Y_START, 0.0);
    commands.spawn((card_boundle, Card {}));
