        ...
        match value {
            ...
            v1 => {
                ...
                for card in ECardNumber::iter() {
                    match card {
                        ECardNumber::Jack | ECardNumber::Queen | ECardNumber::King => {}
                        _ => {
                            let v2 = v1 + card;
                            m.insert(
                                v2,
                                m.get(&v2).unwrap_or(&Fraction::new(0u64, 1u64)).clone() +
                                probability_map.get(&card.into()).unwrap_or(&Fraction::new(0u64, 1u64)).clone()
                            );
                        }
                    }
                }
...
mod test {
    ...
    use super::super::value_hit_distribution::get_table;
